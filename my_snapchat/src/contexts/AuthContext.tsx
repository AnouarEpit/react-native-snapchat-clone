...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (response.success && response.token && response.user) {
        ...
        if (typeof response.token === 'object') {
          console.error('❌ Token es objeto en AuthContext, convirtiendo a string...');
          tokenString = (response.token as any).data?.token || (response.token as any).token || JSON.stringify(response.token);
        }
        ...
        console.log('🔍 Token à sauvegarder (debe ser string):', typeof tokenString, (tokenString as string).substring(0, 50) + '...');
  ...
  const signUp = async (credentials: { email: string; password: string; username: string }) => {
    console.log('📝 Début de l\'inscription pour:', credentials.email, credentials.username);
    setIsLoading(true);
    setError(null);
